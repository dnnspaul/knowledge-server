...
function envErrors(): string[] {
  return validateConfig().filter((e) =>
    FLOAT_VARS.some((v) => e.includes(v))
  );
}
...
describe("validateConfig — DECAY_ARCHIVE_THRESHOLD", () => {
  it("accepts a valid value in (0, 1]", () => {
    ...
    expect(envErrors().filter((e) => e.includes("DECAY_ARCHIVE_THRESHOLD"))).toHaveLength(0);
  });
  ...
  it("rejects 0 (lower bound is exclusive)", () => {
    ...
    expect(envErrors().some((e) => e.includes("DECAY_ARCHIVE_THRESHOLD"))).toBe(true);
  });
...
describe("validateConfig — CONTRADICTION_MIN_SIMILARITY", () => {
  it("accepts a value strictly below 0.82 (RECONSOLIDATION_THRESHOLD)", () => {
    ...
    expect(envErrors().filter((e) => e.includes("CONTRADICTION_MIN_SIMILARITY"))).toHaveLength(0);
  });

  it("rejects 0 (lower bound is exclusive)", () => {
    process.env.CONTRADICTION_MIN_SIMILARITY = "0";
    expect(envErrors().some((e) => e.includes("CONTRADICTION_MIN_SIMILARITY"))).toBe(true);
  });
...
describe("validateConfig — ACTIVATION_SIMILARITY_THRESHOLD", () => {
  it("accepts a valid value in (0, 1]", () => {
    ...
    expect(envErrors().filter((e) => e.includes("ACTIVATION_SIMILARITY_THRESHOLD"))).toHaveLength(0);
  });

  it("rejects 0 (lower bound is exclusive)", () => {
    process.env.ACTIVATION_SIMILARITY_THRESHOLD = "0";
    expect(envErrors().some((e) => e.includes("ACTIVATION_SIMILARITY_THRESHOLD"))).toBe(true);
  });
...
describe("validateConfig — EMBEDDING_DIMENSIONS", () => {
  it("accepts a positive integer", () => {
    ...
    expect(envErrors().filter((e) => e.includes("EMBEDDING_DIMENSIONS"))).toHaveLength(0);
  });
  ...
  it("rejects 0", () => {
    ...
    expect(envErrors().some((e) => e.includes("EMBEDDING_DIMENSIONS"))).toBe(true);
  });
...
describe("validateConfig — multiple errors accumulated", () => {
  it("returns one error per violated constraint", () => {
    ...
    expect(errors.filter((e) => e.includes("DECAY_ARCHIVE_THRESHOLD"))).toHaveLength(1);
    expect(errors.filter((e) => e.includes("CONTRADICTION_MIN_SIMILARITY"))).toHaveLength(1);
    expect(errors.filter((e) => e.includes("EMBEDDING_DIMENSIONS"))).toHaveLength(1);
  });
});
